...
# Create and run 2,000 MODFLOW runs
...
for k in range(0, 2000, 1):
    ...
    nper = 239
    perlen = np.empty((239,), dtype = np.int32)
    perlen[::2] = perlen[::2] = 262
    perlen[1::2] = perlen[1::2] = 103
    nstp = np.empty((239,), dtype = np.int32)
    nstp[::2] = nstp[::2] = 262
    nstp[1::2] = nstp[1::2] = 103
    steady = [False for x in range (1,240)]
    tsmult = np.empty((239,), dtype = np.int32)
    tsmult = 1
    ...
    with open('./Inputs/Eval_Pumping_Rates.csv','r') as f:
        reader = csv.reader(f)
        pr = list(reader)
    ...
    lay_num = np.zeros([64,50])
    row_num = np.zeros([64,50])
    col_num = np.ones([64,50])*list(range(0,50))
    no_pump = np.zeros([64,50])
    ...
    keys_nopump = list(range(112,239,2));
    keys_PR = list(range(111,239,2));
    ...
    with open('./Inputs/Eval_Deep_Percolation.csv','r') as f:
        reader = csv.reader(f)
        inf = list(reader)
    ...
    inf_array = np.zeros([239, 50])
    for i in range(239):
            inf_array[i,0:18] = inf_BAU[i]
            inf_array[i,18:32] = inf_LEMA[i]
            inf_array[i,32:50] = inf_BAU[i]
    
    inf_list = np.split(inf_array, indices_or_sections = 239)
    ...
    # of each month #
    spinup = [i for i in range(0,239,2) for _ in range(8)]
    spinup2 = [i for i in range(1,239,2) for _ in range(4)]
    ...
    Path('./Data/Evaluation/%s' % (modelname)).mkdir(parents = True, 
                                                          exist_ok = True)
    os.chdir('./Data/Evaluation/%s/' % (modelname))
